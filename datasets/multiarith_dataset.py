...
def multiarith_data_process(dataset):
    """
    Convert raw MultiArith JSON list into the format expected by the runner:
    each item -> {"task": <question>, "answer": <normalized_answer_string>}
    """
    list_data_dict = []
    for data in dataset:
        # Question text
        ...
        # Answer can be under "final_ans" (ChilleD/MultiArith) or "answer"
        raw_answer = data.get("final_ans", data.get("answer", ""))
...
def multiarith_get_predict(pred_str):
    ...
    # MultiArith answers are integers; grab the last integer
    if pred.isdigit():
        return pred
    else:
...
def multiarith_check_correctness(pred, gt):
    """
    Compare predicted answer vs ground-truth for MultiArith.
